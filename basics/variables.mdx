...
- `"Amit"` is the value we're storing
...
**Allowed:**
```python
user_name = "Dev"      # lowercase with underscores (Python style)
userName = "Dev"       # camelCase (works but not Python style)
...
my-name = "Dev"       # No hyphens (Python thinks it's subtraction)
my name = "Dev"       # No spaces
...
firstName = "Amit"  # Works, but not Python style
userAge = 25
isLoggedIn = True
```
...
<AccordionGroup>
  <Accordion title="Forgetting quotes around text">
    ```python
    # Wrong
    name = Amit  # Python looks for a variable called Amit

    # Right
    name = "Amit"  # This creates text
    ```
  </Accordion>
