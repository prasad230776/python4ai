...
# 1. Using np.newaxis
row_vec_new = a[np.newaxis, :]    # Shape: (1, 6) (Row Vector)
col_vec_new = a[:, np.newaxis]    # Shape: (6, 1) (Column Vector)

# 2. Using np.expand_dims
row_vec_expand = np.expand_dims(a, axis=0)  # Shape: (1, 6)
col_vec_expand = np.expand_dims(a, axis=1)  # Shape: (6, 1)
```
...
All default arithmetic operations are executed **element-by-element**.
* **Addition (`+`)**: Computes element-wise sum.
* **Subtraction (`-`)**: Computes element-wise difference.
* **Multiplication (`*`)**: Performs element-wise multiplication (this is *not* matrix multiplication).
* **Division (`/`)**: Computes element-wise division.

*Constraint:* The arrays must have compatible shapes for broadcasting, or a `ValueError` is raised.
...
You know NUmpy! Let's dive deeper into working next library Pandas.
